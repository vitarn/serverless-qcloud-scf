...
module.exports = {
  ...
  createBucketIfNotExists() {
    ...
    const bucket = templates.create.Resources.ServerlessDeploymentBucket;

    return provider.headBucket(bucket.Properties.BucketName, bucket.Properties.Region)
      .then(headBucket => {
        if (headBucket.exist) { 
          cli.log(`Bucket ${bucket.name} already exists.`);
          return headBucket;
        }
        cli.log(`Creating bucket ${bucket.BucketName}...`);
        return provider.createBucket(bucket.BucketName)
          .then(() => {
            cli.log(`Created bucket ${bucket.BucketName}`);
          });
      }).then((foundBucket) => {
        provider.resetOssClient(bucket.BucketName);
      });
  }
};
